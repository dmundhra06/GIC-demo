...
def run_query(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    conn.row_factory = sqlite3.Row
    cursor = conn.execute(EXECUTION_SEQUENCE_SQL)
    return [dict(row) for row in cursor.fetchall()]

def print_results(results: list[dict[str, Any]]) -> None:
    print(f"{'UNIT_NBR':<10} {'STEP_SEQ_ID':<13} {'EXEC_LEVEL':<12} {'STEP_PROG_NAME'}")
    print("-" * 100)
    for row in results:
        print(
            f"{row['UNIT_NBR']:<10} {row['STEP_SEQ_ID']:<13} "
            f"{row['execution_level']:<12} {row['STEP_PROG_NAME']}"
        )
